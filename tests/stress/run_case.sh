...
if [[ ! -d "${CASE_DIR}" ]]; then
  echo "[stress] case directory not found: ${CASE_DIR}" >&2
  exit 1
fi

MANIFEST_PATH="${CASE_DIR}/Cargo.toml"
if [[ ! -f "${MANIFEST_PATH}" ]]; then
  echo "[stress] missing Cargo.toml in ${CASE_DIR}" >&2
  exit 1
fi
...
if status not in {"pass", "fail"}:
    print("[stress] missing 'status' field (pass|fail) in output", file=sys.stderr)
    sys.exit(1)

result_path.write_text(json.dumps(data, indent=2))
print(f"[stress] stored result -> {result_path}")
